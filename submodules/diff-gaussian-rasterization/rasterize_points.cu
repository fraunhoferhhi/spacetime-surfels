	...
	const torch::Tensor& config)
{
  ...
  return std::make_tuple(rendered, out_color, out_normal, out_depth, out_opac, radii, geomBuffer, binningBuffer, imgBuffer, proj_2D, conic_2D, conic_2D_inv, gs_per_pixel, weight_per_gs_pixel, x_mu);
}
	...
	const torch::Tensor& config) 
{
  ...
  {  
	  CudaRasterizer::Rasterizer::backward(P, degree, M, R,
		...
		prcppoint.contiguous().data_ptr<float>(),
		patchbbox.contiguous().data_ptr<float>(),
		tan_fovx,
		tan_fovy,
		radii.contiguous().data<int>(),
		...
		dL_dout_opac.contiguous().data_ptr<float>(),
		dL_dmeans2D.contiguous().data_ptr<float>(),
		dL_dconic.contiguous().data_ptr<float>(),  
		dL_dopacity.contiguous().data_ptr<float>(),
		dL_dcolors.contiguous().data_ptr<float>(),
		dL_dnormal.contiguous().data_ptr<float>(), // to store intermediate results, not for input or output
		dL_ddepth.contiguous().data_ptr<float>(), // to store intermediate results, not for input or output
		dL_proj_2D.contiguous().data_ptr<float>(),
		dL_conic_2D.contiguous().data_ptr<float>(),
		dL_conic_2D_inv.contiguous().data_ptr<float>(),
		dummy_gs_per_pixel.contiguous().data_ptr<float>(),
		dummy_weight_per_gs_pixel.contiguous().data_ptr<float>(),
		grad_x_mu.contiguous().data_ptr<float>(),
		...
		debug, 
		config.contiguous().data_ptr<float>());
  }
