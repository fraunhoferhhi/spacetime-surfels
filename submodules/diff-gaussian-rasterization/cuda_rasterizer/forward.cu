	...
	float* config)
{
	...
	float4 p_hom = transformPoint4x4(p_orig, projmatrix); //clip space
	float p_w = 1.0f / (p_hom.w + 0.0000001f);
	float3 p_proj = { p_hom.x * p_w, p_hom.y * p_w, p_hom.z * p_w }; // ndc space
	...
	if (surface) {
		float3 n_view   = transformVec4x3({R[0][2], R[1][2], R[2][2]}, viewmatrix); // z-axis of Gaussian in view space
		float3 ax0_view = transformVec4x3({R[0][0], R[1][0], R[2][0]}, viewmatrix); // x-axis in view space
	...
	float* config)
{
	...
	float D_buffer[D_buffer_size], W_buffer[D_buffer_size], depth_temp, pid_buffer[D_buffer_size];
	    //   axDif_buffer[D_buffer_size * 2],
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < CHANNELS; ch++) 
				C[ch] += collected_feature[ch * BLOCK_SIZE + j] * w;
			
			if (surface) 
				for (int ch = 0; ch < 3; ch++) 
					N[ch] += collected_normal[ch * BLOCK_SIZE + j] * w;			
