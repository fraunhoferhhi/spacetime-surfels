	...
	float* config)
{
	...
	CHECK_CUDA(BACKWARD::render(
		...
		dL_dpixdepth,
		dL_dpixopac,
		// above are inputs (gradient w.r.t. rendered maps), below are outputs (gradient w.r.t. 2D Gaussians)
		(float3*)dL_dmean2D,
		(float4*)dL_dconic,
		dL_dopacity,
		dL_dcolor,
	...
	CHECK_CUDA(BACKWARD::preprocess(P, D, M,
		...
		dL_dnormal,
		dL_ddepth,
		// above are inputs (gradient w.r.t. 2D Gaussians), below are outputs (gradient w.r.t. 3D Gaussians)
		dL_proj_2D,
		dL_conic_2D,
		dL_conic_2D_inv,
		dummy_gs_per_pixel,
