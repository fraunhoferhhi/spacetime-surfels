...
__global__ void computeCov2DCUDA(
	...
	const int* radii,
	const float* cov3Ds,
	const float h_x, float h_y, // actually fx, fy
	...
	float* config)
{
	...
	float3 dL_dmean2d = {dL_dmean2D[idx].x, dL_dmean2D[idx].y, 0};
	...
	float* config)
{
	...
	float accum_rec[C] = { 0 }, accum_rec_n[3] = {0}, accum_rec_d = 0, accum_rec_o = 0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float alpha = min(0.99f, con_o.w * G); // opacity is packed together with conic (inv cov) in the forward pass: conic_opacity[idx] = { conic.x, conic.y, conic.z, opacities[idx] } -> con_o.w = opacities[idx]
			...
			if (surface) {
				for (int ch = 0; ch < 3; ch++) { // grads w.r.t. tangent normal	
					...
					// Atomic, since this pixel is just one of potentially
					// many that were affected by this Gaussian.
					atomicAdd(&(dL_dnormal[global_id * 3 + ch]), dchannel_dcolor * dL_dchannel * 10); // == lr * 10
			...
			for (int ch = 0; ch < 1; ch++) { // grads w.r.t. depth
				...
				if (normalize_depth) { // Additional gradients w.r.t. alpha from depth normalization
					// because out_depth[pix_id] = normalize_depth ? D / (1 - T) : D + T * 10;
			...
			// Add grads w.r.t. alpha from opac output from mask loss
			...
			if (!normalize_depth) dL_dalpha += (-T_final / (1.f - alpha)) * (10 * dL_dpixD); // because out_depth[pix_id] = normalize_depth ? D / (1 - T) : D + T * 10;
			...
			atomicAdd(&dL_proj_2D[global_id * 2 + 0], dL_dNDC.x);
			atomicAdd(&dL_proj_2D[global_id * 2 + 1], dL_dNDC.y);
			...
			atomicAdd(&dL_conic_2D[global_id * 3 + 0], dL_dconic.x);
			atomicAdd(&dL_conic_2D[global_id * 3 + 1], dL_dconic.y);
			atomicAdd(&dL_conic_2D[global_id * 3 + 2], dL_dconic.z);
