...
# # prcp is not considered!
# def getProjectionMatrix(znear, zfar, fovX, fovY, prcp):
#     tanHalfFovY = math.tan((fovY / 2)) # == H/(2*fy)
#     tanHalfFovX = math.tan((fovX / 2)) # == W/(2*fx)
...
#     h = top - bottom # == znear*H/fy
#     w = right - left # == znear*W/fx

#     P = torch.zeros(4, 4)

#     z_sign = 1.0

#     P[0, 0] = 2.0 * znear / w # == 2*fx/W
#     P[1, 1] = 2.0 * znear / h # == 2*fy/H
#     P[0, 2] = -(right + left) / w # 0
#     P[1, 2] = -(top + bottom) / h # 0
#     P[3, 2] = z_sign
