...
# Code taken and adapted from MAGS: https://github.com/jasongzy/MAGS/blob/d8186ada161ab0f058138f63c83a765ab64abaed/utils/flow_utils.py
...
def warp_flow(img, flow):
    ...
    flow_new = flow_new.permute(1, 2, 0).unsqueeze(0)  # Reshape for grid_sample and add batch dimension
...
def compute_fwdbwd_mask(fwd_flow, bwd_flow):
    ...
    fwd_lr_error = torch.norm(fwd_flow + bwd2fwd_flow, dim=0)  # cycle error of img1->img2->img1
    fwd_mask = fwd_lr_error < alpha_1 * (torch.norm(fwd_flow, dim=0) + torch.norm(bwd2fwd_flow, dim=0)) + alpha_2  # mask of img1

    fwd2bwd_flow = warp_flow(fwd_flow, bwd_flow)
    bwd_lr_error = torch.norm(bwd_flow + fwd2bwd_flow, dim=0)

    bwd_mask = bwd_lr_error < alpha_1 * (torch.norm(bwd_flow, dim=0) + torch.norm(fwd2bwd_flow, dim=0)) + alpha_2
...
def pixel_to_gaussian_idx(
    ...
    nearby_rotations = gaussians.get_full_rotation(timestamp)[is_fg][knn_idx]
    nearby_scales = gaussians.get_scaling[is_fg][knn_idx]
    nearby_opacities = gaussians.get_base_opacity[is_fg][knn_idx]
...
def render_flow(flow_project: torch.Tensor, render_fn) -> torch.Tensor:
    ...
    flow_render = render_pkg_flow["render"]
    return flow_render
...
def prepare_flow_for_visualization(flow_fwd_per_gs, seg_map_indices, fg_gaussian_contrib, image_shape):
    """
    Converts gaussian flow data into a full image for visualization, computed on GPU
    
    Args:
        flow_fwd_per_gs: Flow per gaussian with shape (N*K, 2) as PyTorch tensor
        seg_map_indices: Indices of valid pixels in the flattened image
        fg_gaussian_contrib: Weights for each gaussian with shape (N, K)
        image_shape: Tuple (h, w) of the image dimensions
        
    Returns:
        torch.Tensor: Flow image with shape (h, w, 2)
    """
    h, w = image_shape
    device = flow_fwd_per_gs.device
    
    # Create empty flow image as a PyTorch tensor
    flow_image = torch.zeros((h, w, 2), device=device)
    
    # Calculate K (gaussians per pixel)
    k = flow_fwd_per_gs.shape[0] // len(seg_map_indices)
    
    # Process each valid pixel
    for i, idx in enumerate(seg_map_indices):
        y, x = idx.item() // w, idx.item() % w
        
        # Get the K gaussians for this pixel
        start_idx = i * k
        end_idx = (i + 1) * k
        
        pixel_flows = flow_fwd_per_gs[start_idx:end_idx]
        pixel_weights = fg_gaussian_contrib[i, :]
        
        # Normalize weights and compute weighted flow
        weight_sum = torch.sum(pixel_weights)
        if weight_sum > 0:
            # Use torch operations for weighted sum
            flow_image[y, x] = torch.sum(pixel_flows * (pixel_weights / weight_sum), dim=0)
    
    return flow_image

def get_flow_images(gt_flow_fwd, gt_flow_bwd, seg_map, flow_fwd_per_gs, flow_bwd_per_gs, fg_gaussian_contrib):
    seg_mask = seg_map.cpu().numpy()
    gt_fwd_flow_viz = flow_viz.flow_to_image(gt_flow_fwd.cpu().numpy().transpose(1, 2, 0)) / 255.0
    gt_fwd_flow_viz *= np.stack([seg_mask, seg_mask, seg_mask], axis=-1)
    gt_bwd_flow_viz = flow_viz.flow_to_image(gt_flow_bwd.cpu().numpy().transpose(1, 2, 0)) / 255.0
    gt_bwd_flow_viz *= np.stack([seg_mask, seg_mask, seg_mask], axis=-1)

    seg_indices = torch.nonzero(seg_map.reshape(-1)).squeeze()
    image_shape = gt_flow_fwd.shape[1:3]

    pred_fwd_flow = prepare_flow_for_visualization(flow_fwd_per_gs, seg_indices, fg_gaussian_contrib, image_shape)
    pred_fwd_flow_viz = flow_viz.flow_to_image(pred_fwd_flow.cpu().numpy()) / 255.0

    pred_bwd_flow = prepare_flow_for_visualization(flow_bwd_per_gs, seg_indices, fg_gaussian_contrib, image_shape)
    pred_bwd_flow_viz = flow_viz.flow_to_image(pred_bwd_flow.cpu().numpy()) / 255.0

    device = flow_fwd_per_gs.device

    return torch.Tensor(gt_fwd_flow_viz.transpose(2, 0, 1)).to(device), torch.Tensor(pred_fwd_flow_viz.transpose(2, 0, 1)).to(device), torch.Tensor(gt_bwd_flow_viz.transpose(2, 0, 1)).to(device), torch.Tensor(pred_bwd_flow_viz.transpose(2, 0, 1)).to(device)